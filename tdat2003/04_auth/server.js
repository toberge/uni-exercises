...
// const TOKEN_EXPIRE_TIME = 60*30; TODO uncomment for more appropriate timing
const TOKEN_EXPIRE_TIME = 5;
...
app.post("/login", (req, res) => {
  console.log(req.body.brukernavn, 'tried to log in');
  if (loginOk(req.body.brukernavn, req.body.passord)) {
    ...
    let token = jwt.sign({ brukernavn: req.body.brukernavn }, privateKey, {
      expiresIn: TOKEN_EXPIRE_TIME // kan økes til f.eks. halvtime
    });
    res.json({ jwt: token });
  } else {
...
app.get("/token", (req, res) => {
  ...
  jwt.verify(token, publicKey, (err, decoded) => {
    if (err) {
      ...
      console.log('REGENERATNG...');
      token = jwt.sign({ brukernavn: decoded.brukernavn }, privateKey, {
        expiresIn: TOKEN_EXPIRE_TIME // kan økes til f.eks. halvtime
      });
